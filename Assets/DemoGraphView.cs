...
public class DemoGraphView : RuntimeGraphView
{
    protected override void Awake()
    {
        base.Awake();
        menu.Insert(0, new ContextMenuSeparator());
        menu.Insert(0, new ContextMenuItem("Add Node", () => AddNode<TestNodeModelSingle>(), () => true));

        
        var node = AddNode<TestNodeModelSingle>();
        node.SetPosition(new Vector2(300, -300));
        node = AddNode<TestNodeModelMulti>();
        node.SetPosition(new Vector2(600, -300));
        node = AddNode<TestNodeModelSingle>();
        node.SetPosition(new Vector2(900, -300));
        node = AddNode<TestNodeModelSingle>();
        node.SetPosition(new Vector2(900, -600));
